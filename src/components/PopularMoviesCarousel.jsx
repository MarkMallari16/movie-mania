    ...
    return (
        <div className='carousel carousel-center flex gap-5  rounded-lg mt-10'>
            {popular.results.map(popularMovie => (
                <div className='carousel-item ' key={popularMovie.id}>
                    <Link to={`/movie/${popularMovie.id}`} className='relative'>
                        <img src={`https://image.tmdb.org/t/p/w500${popularMovie.backdrop_path}`} alt={popularMovie.title} className='rounded-lg opacity-75' />
                        <div className=''>
                            <p className='absolute bottom-4 start-3 text-white text-xl'>{popularMovie.title}</p>
